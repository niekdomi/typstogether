  ...
  [
    ...
    createAnimationRule(
      ...
      (val, theme) =>
        val === "initial"
          ? val
          : (theme.duration?.[val] ?? theme.duration?.DEFAULT ?? h.bracket.cssvar.time(val)),
...
export const animationDirectionRules: Rule<Theme>[] = [
  [
    ...
    createAnimationRule(
      ...
      "animation-direction",
      (val) =>
        ["normal", "reverse", "alternate", "alternate-reverse", "initial"].includes(val)
          ? val
          : undefined,
      { syntax: "*", inherits: false, initialValue: "normal" }
    ),
    {
      autocomplete: "direction-(normal|reverse|alternate|alternate-reverse|initial)",
    },
  ],
];
...
export const animationFillModeRules: Rule<Theme>[] = [
  [
    ...
    createAnimationRule(
      ...
      "animation-fill-mode",
      (val) =>
        ["none", "forwards", "backwards", "both", "initial"].includes(val) ? val : undefined,
...
const createSlideRule = (prefix: "enter" | "exit") => {
  const propBase = prefix === "enter" ? "--un-in-translate" : "--un-out-translate";
  return function* (
    ...
    if (["start", "end"].includes(dir)) {
      ...
      if (vBracket) {
        ...
        yield {
          [symbols.selector]: (s: string) => `${s}:where(:dir(ltr), [dir="ltr"], [dir="ltr"] *)`,
          [prop]: ltrVal,
        };
        yield {
          [symbols.selector]: (s: string) => `${s}:where(:dir(rtl), [dir="rtl"], [dir="rtl"] *)`,
          [prop]: rtlVal,
        };
        return;
      }
      ...
      if (vNumber) {
        ...
        const ltrVal = `calc(${vNumber} * ${spacing}${dir === "start" ? " * -1" : ""})`;
        const rtlVal = `calc(${vNumber} * ${spacing}${dir === "start" ? "" : " * -1"})`;
        yield {
          [symbols.selector]: (s: string) => `${s}:where(:dir(ltr), [dir="ltr"], [dir="ltr"] *)`,
          [prop]: ltrVal,
        };
      ...
      if (vFraction) {
        yield define();
        const ltrVal = `calc(${vFraction}${dir === "start" ? " * -100%" : " * 100%"})`;
        const rtlVal = `calc(${vFraction}${dir === "start" ? " * 100%" : " * -100%"})`;
        yield {
          [symbols.selector]: (s: string) => `${s}:where(:dir(ltr), [dir="ltr"], [dir="ltr"] *)`,
          [prop]: ltrVal,
        };
    ...
    if (vFraction) {
      yield define();
      yield {
        [prop]: `calc(${vFraction}${["top", "left"].includes(dir) ? " * -100%" : " * 100%"})`,
      };
      return;
    }
  };
...
export const slideRules: Rule<Theme>[] = [
  ...
  [/^slide-out-from-(top|bottom|left|right|start|end)(?:-(.+))?$/, createSlideRule("exit")],
];
