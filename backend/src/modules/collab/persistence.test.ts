...
describe("fetchDocument", () => {
  test("returns null when no document exists for the project", async () => {
    ...
    expect(await fetchDocument(project.id)).toBeNull();
  });
  ...
  test("returns the state that was stored", async () => {
    ...
    await storeDocument(project.id, state);

    expect(await fetchDocument(project.id)).toEqual(state);
  });
});
...
describe("storeDocument", () => {
  test("upserts: second call replaces state", async () => {
    ...
    await storeDocument(project.id, new Uint8Array([1, 2, 3]));
    await storeDocument(project.id, new Uint8Array([4, 5, 6]));

    expect(await fetchDocument(project.id)).toEqual(new Uint8Array([4, 5, 6]));
  });
});
